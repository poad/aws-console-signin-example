...
const UserDetail = ({
  ...
  const [detailErrorDialog, setDetailErrorDialog] = useState<
    DetailErrorDialog | undefined
  >(undefined);
  ...
  const {
    ...
    deleteGroup,
    clearAttacheRole,
  } = useGroupDetail(client, group, onUpdate, onDelete);
  ...
  const handleRoleChange = (event: SelectChangeEvent<string>) =>
    changeGroupRole(event.target.value as string);
  ...
  return (
    <Container sx={{ width: '100%' }}>
          ...
          {detailErrorDialog?.title !== undefined ? (
            <DialogTitle id="user-detail-dialog-title">
              <Typography variant="h4" component="span" gutterBottom>
                {detailErrorDialog.title}
              </Typography>
            </DialogTitle>
          ) : undefined}
          <DialogContentText id="alert-dialog-description" component="div">
            {detailErrorDialog?.message !== undefined
              ? detailErrorDialog.message
              : ''}
          </DialogContentText>
        </DialogContent>
      </Dialog>
            ...
            <Container>
              ...
              >
                ...
                <ReadOnlyTextField
                  ...
                  variant="outlined"
                  key="createdAt"
                  defaultValue={detail?.creationDate?.toLocaleString()}
                />
                <ReadOnlyTextField
                  id="lastModifiedAt"
                  ...
                  variant="outlined"
                  key="lastModifiedAt"
                  defaultValue={detail?.lastModifiedDate?.toLocaleString()}
                />
                <FormControl
                  variant="outlined"
                  fullWidth
                ...
                <ReadOnlyTextField
                  ...
                  key="precedence"
                  variant="outlined"
                  defaultValue={detail?.precedence}
                />
              ...
              <DialogActions>
                <Button autoFocus sx={{ margin: 8 }} onClick={onClose}>
                  CLOSE
                </Button>
                <Button
                  sx={{ margin: 8 }}
      ...
      <Dialog
        ...
        aria-describedby="alert-dialog-description"
      >
        <DialogTitle id="alert-group-role-dialog-title">
          {'Attache IAM role to Group?'}
        </DialogTitle>
        <DialogContent>
          <DialogContentText id="alert-group-role-dialog-description">
            Once the IAM role is set for a group, it cannot be reverted to the
            state where the IAM role is not set, is that correct?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
