...
export class LambdaExamplesStack extends Stack {
  constructor(scope: Construct, id: string, props: LambdaExamplesStackProps) {
          ...
          'logs-policy': new PolicyDocument({
            statements: [
              new PolicyStatement({
                ...
                resources: [
                  `arn:aws:logs:${this.region}:${this.account}:log-group:/aws/lambda/${props.Lambda.app.signIn.name}:*`,
                ],
              }),
            ],
          }),
          ...
          'logs-policy': new PolicyDocument({
            statements: [
              new PolicyStatement({
                ...
                resources: [
                  `arn:aws:logs:${this.region}:${this.account}:log-group:/aws/lambda/${props.Lambda.app.signOut.name}:*`,
                ],
              }),
            ],
          }),
          ...
          'logs-policy': new PolicyDocument({
            statements: [
              new PolicyStatement({
                ...
                resources: [
                  `arn:aws:logs:${this.region}:${this.account}:log-group:/aws/lambda/${props.Lambda.app.userInfo.name}:*`,
                ],
              }),
            ],
          }),
    ...
    api.addRoutes({
      path: '/userInfo',
      methods: [HttpMethod.ANY],
      integration: new HttpLambdaIntegration('user-info-handler', userInfoFn),
    });
    ...
    const roles = props.groups.map((group) => {
      const groupRole = new Role(
        this,
        `${props.environment}-GroupRole-${group.name}`,
        {
